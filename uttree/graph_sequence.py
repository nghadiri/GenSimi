# -*- coding: utf-8 -*-
"""
Graph Sequence Generation - Alternative UTTree Implementation

This module provides an alternative graph-based approach for generating
patient sequences in the UTTree methodology using NetworkX graphs.
...
Alternative Graph Construction Approach:

1. Patient-Centered Graph Creation:
   - Creates NetworkX graphs with patient ID as root node
   - Adds subgraph nodes for different time periods/data types
   - Connects temporal nodes in sequential order

2. Medical Event Integration:
   - Incorporates procedure, drug, and medical history events
   - Creates hierarchical connections: Patient → Subgraph → Time → Events
   - Maintains temporal ordering within each subgraph

3. Graph Traversal and Sequence Generation:
   - Uses Breadth-First Search (BFS) for consistent traversal
   - Applies stopword filtering to remove structural elements
   - Generates clean sequences containing only medical content

4. Alternative Document Generation Methods:
   - Multiple sequence generation approaches (S_Non_TT, S_TT, S_U_TT, etc.)
   - Combines structured and unstructured data in different ways
   - Creates various document representations for comparison

Key Differences from Main UTTree Pipeline:
- Uses NetworkX graphs instead of custom tree structures
- Alternative BFS traversal implementation
- Multiple document generation strategies
- Integration with ICD codes and medical history

This alternative implementation provides flexibility in graph construction
and sequence generation, allowing for experimental variations of the
core UTTree methodology while maintaining the fundamental approach of
temporal medical event representation.

Input: Stage CSV files with quadruple temporal data
Output: Patient sequences for document embedding
"""
